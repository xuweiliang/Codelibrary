...
horizon.addInitFunction(horizon.instances.init = function () {
  ...
  function update_launch_source_displayed_fields (field) {
    ...
    switch(base_type) {
      case "image_id":
        elements_list = "#id_image_id, #id_count,#id_name, #id_pool";
        break;
      case "templet_id":
        elements_list = "#id_image_or_snapshot, #id_templet_id, #id_flavor";
        $("#id_name, #id_image_id, #id_count, #id_input_user,"+
                      " #id_pool").closest(".form-group").hide();
        break;
      case "custom_id":
        elements_list = "#id_flavor, #id_name, #id_pool, #id_flavor, "+
                        "#id_count, #id_source_type, #id_image_or_snapshot, #id_input_user";
        break;
      case "instance_snapshot_id":
        elements_list = "#id_instance_snapshot_id, #id_count,#id_name, #id_pool";
        break;
      case "volume_id":
        elements_list = "#id_volume_id, #id_device_name, #id_vol_delete_on_instance_delete,"+
                        " #id_count,#id_name, #id_pool";
        break;
  ...
  /*
   * Instance time switch machine control function Xu Weiliang 2017/4/11 added
  */

    function terminal_control_displayed_fields(field){
        var $this = $(field);
        var base_type = $this.val();
        var base_id = $this.attr("id");
        var elements_show, elements_hide
    	if( base_id == "id_multi_user"){
    		if( base_type == 'on'){
    		    elements_show = "#id_broadcast";
    		}else{
    	            elements_hide = "#id_broadcast";
    		}
    	}else if( base_id == "id_shutdown_restore" ){
    		if( base_type == 'no'){
                    elements_show = "#id_during";
    		}else{
                    elements_hide = "#id_during";
    		}
    	}else if (base_id =="id_during"){
                if(base_type == 'variable'){
                    elements_show="#id_per"
                }else{
                    elements_hide="#id_per"
                }
        }else if (base_id =="id_per"){
                if(base_type == 'day_id'){
                    elements_show="#id_day_id"
                    elements_hide="#id_week_id, #id_month_id"
                }else if(base_type == 'week_id'){
                    elements_show="#id_week_id"
                    elements_hide="#id_day_id, #id_month_id"
                }else if(base_type == 'month_id'){
                    elements_show="#id_month_id"
                    elements_hide="#id_day_id, #id_week_id"
                }
        }
        var elements_show_group=$(elements_show).closest(".form-group");
        var elements_hide_group=$(elements_hide).closest(".form-group");
            elements_show_group.show()
            elements_hide_group.hide()
    }


  horizon.modals.addModalInitFunction(function (modal) {
    var hide_div = "#id_broadcast, #id_per, "+
          "#id_month_id, #id_day_id, #id_week_id"
    $(modal).find(hide_div).closest(".form-group").hide()
    $(".workflow #id_multi_user").on("change",function(){
          terminal_control_displayed_fields(this);
    })
    $(".workflow #id_shutdown_restore").on("change",function(){
         terminal_control_displayed_fields(this);
    })
    $(".workflow #id_during").on("change",function(){
         terminal_control_displayed_fields(this);
    })
    $(".workflow #id_per").on("change",function(){
         terminal_control_displayed_fields(this);
    })
  ...
  // end terminal control displayed field
  ...
  horizon.modals.addModalInitFunction(function (modal) {
    $(modal).find("#id_timing_mode").closest(".form-group").nextAll().hide()
    //$("#id_timing_mode").closest(".form-group").nextAll().hide();
...
//     $('#id_loop').closest(".form-group").hide();
     if (timing_mode == 'single'){
         ...
         var colSmAdd = $(".col-sm-6").eq(0)
         colSmAdd.append("<div id='week' style='font-size:12px; letter-spacing: 0.8px;'>"+
         ...
         $("#week a").css("border-radius","6px");
         $("#week a").css("color","inherit");
         $("#week a").css("text-decoration","none");
         var vals = [];
