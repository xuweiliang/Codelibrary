...
class DataTable(object):
    ...
    def get_prev_marker(self):
        ...
        """
#        page = urlencode({'page':self._meta.page_index-1})
#        return http.urlquote_plus('&'.join([str(self.get_object_id(self.data[0])), page])) \
#            if self.data else ''
    ...
    def get_marker(self):
        ...
        LOG.info("KKKKKKKKKKKKKKKKKKKKK %s" % self._meta.page_index)
        #LOG.info("MMMMMMMMMMMMMMMMMMMMMMM %s" % int(self._meta.page_index))
        page=urlencode({'page':self._meta.page_index+1})
        LOG.info("MMMMMMMMMMMMMMMMMMMMMMM %s" % page)
#        return http.urlquote_plus('&'.join([str(self.get_object_id(self.data[-1])), page])) \
#            if self.data else ''
