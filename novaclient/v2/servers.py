# Coeyright 2010 Jacob Kaplan-Moss
...
class Server(base.Resource):
    ...
    def reallocate(self, project=None, user=None):
        """Allocation instance
           :param project, user 
           returns : class: Server"""
        return self.manager.reallocate(self, project=project, user=user)
...
class ServerManager(base.BootingManagerWithFind):
    ...
    def _boot(self, resource_url, response_key, name, image, pool, flavor,
              meta=None, files=None, userdata=None, terminal=None,
...
#        return result
    ...
    def get_object_info(self, server):
        return self._action('object_info', server, None)

    def get_spice_secure(self, server):
        return self._action('secure', server, None)

    def get_vm_password(self, server):
        return self._action('os-vmpwd', server, None)

    def create_remote(self, remote=None):
        LOG.info("kwargs ================%s" % remote)
        return self._action('remote', None, remote)

    def cancel_remote_assistance(self, server):
        return self._action('cancel_assistance', server, None)

    def remote_assistance_list(self, instance_id=None):
        body = {'instance_id': instance_id}
        return self._action('remote_list', None, body)

    def reallocation(self, server, project=None, user=None):
        """
        Allocation instance
        param project and user
        returns:An instance of novaclient.base.TupleWithMeta 
        """
    ...
    def dev_snapshot_list(self, server):
        return self._list('/servers/%s/os-devsnapshotlist' % base.getid(server), 'devsnapshotslist')
    ...
    def create(self, name, image, pool, flavor, meta=None, files=None,
               reservation_id=None, min_count=None, terminal=None,
        ...
        boot_args = [name, image, pool, flavor]
        ...
        boot_kwargs = dict(
            meta=meta, files=files, userdata=userdata,terminal=terminal,
    ...
    @api_versions.wraps("2.0", "2.18")
    def update(self, server, name=None, terminal=None):
        """
    ...
    def timing_interface(self, data=None):
        """Instance method of time switch machine"""
        if not data:
            return
        timing_boot = None
        timing_boot_time = None
        if "timing_boot" in data:
            timing_boot = data['timing_boot']
            timing_boot_time = data['timing_time']

        timing_shutdown = None
        timing_shutdown_time = None
        if "timing_shutdown" in data:
            timing_shutdown = data['timing_shutdown']
            timing_shutdown_time = data['timing_time']
        body ={
            "server":{'timing_boot':timing_boot,
                      'timing_boot_time':timing_boot_time,
                      'timing_shutdown':timing_shutdown,
                      'timing_shutdown_time':timing_shutdown_time,
                      'checkboxlist':data['checkboxlist']}
        }
        return self._action('timing', None, body)
