...
class ResolutionDialog(wx.Dialog):
    def __init__(self, parent, msg, data, local):
    ...
    def TimedOut(self):
        if self.local != 'true':
            ...
            fileSrc = "%s/xorg/xorg.conf_%s" %(currentPath, (self.data))
            fileDest = "/etc/X11/xorg.conf"
            command = "cp -f"+" "+fileSrc+" "+fileDest
            os.system(command)
        os.system("sync")
        os.system("reboot")
...
if __name__ == '__main__':
    app = wx.PySimpleApp()
    Resource.load()
    dlg = ResolutionDialog(None, 'wewe', )
