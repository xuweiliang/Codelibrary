...
from nova.record import recordinfo
from nova.record import instance_actions
#from nova.db.base import *
...
class ServersController(wsgi.Controller):
    ...
    def _get_servers(self, req, is_detail):
        ...
        if all_tenants:
            ...
            if 'project_id' not in search_opts:
    ...
    def create(self, req, body):
        ...
        cipher = server_dict['cipher']
        pool = server_dict['pool']
        LOG.info("cipher ======================%s" % cipher)
        terminal = None
        if server_dict.get('terminal', None):
            terminal = server_dict['terminal'].pop()
    ...
    def update(self, req, id, body):
        ...
        server = body['server']
        terminal = {}
        if 'terminal' in server:
            terminal=server.get('terminal').pop()
        if 'name' in server:
            update_dict['display_name'] = common.normalize_name(
                server['name'])
            recordinfo.add_current_record_for_success(ctxt, body['server']['name'],
                                                      instance_actions.UPDATE)


        if 'user' in server:
            user_id = server.get('user', None)
            update_dict['user_id'] = user_id
     
        if 'project' in server:
            project_id = server.get('project', None)
        ...
        if 'usb_control' in terminal:
            usb_control = terminal['usb_control']
            if usb_control == instance.usb_control:
                pass
            else:
                update_dict['usb_control'] = usb_control
                self.compute_api.update_usb(ctxt, instance)
        if 'screen' in terminal:
            screen = terminal['screen']
            if screen == str(instance.screen):
                pass
            else:
                update_dict['screen'] = screen
                self.compute_api.update_screen(ctxt, instance)


        if 'spice_secure' in terminal:
            spice_secure = terminal['spice_secure']
            if spice_secure == instance.spice_secure:
                pass
            else:
                update_dict['spice_secure'] = spice_secure
                self.compute_api.update_spice_secure(ctxt, instance)

        if 'clipboard_control' in terminal:
            clipboard_control = terminal['clipboard_control']
            if clipboard_control == instance.clipboard_control:
                pass
            else:
                update_dict['clipboard_control'] = clipboard_control
                self.compute_api.update_clipboard(ctxt, instance)
        if 'multi_user' in terminal:
            multi_user = terminal['multi_user']
            update_dict['multi_user'] = multi_user

        if 'allow_screen_broadcast' in terminal:
            allow_screen_broadcast = terminal['allow_screen_broadcast']
            update_dict['allow_screen_broadcast'] = allow_screen_broadcast

        if 'client_ip' in body['server']:
            client_ip = body['server']['client_ip']
            update_dict['client_ip'] = client_ip

        if 'client_time' in body['server']:
            client_time = body['server']['client_time']
            update_dict['client_time'] = client_time

        if 'quatity_control' in terminal:
            quatity_control = terminal['quatity_control']
            update_dict['quatity_control'] = quatity_control

        if 'jostle' in terminal:
            jostle = terminal['jostle']
            if jostle == instance.jostle:
                pass
            else:
                update_dict['jostle'] = jostle
        if 'during' in terminal:
            during = terminal['during']
            if during == instance.during:
                pass
            else:
                update_dict['during'] = during
        ...
        if 'dev_time' in terminal:
            dev_time = terminal['dev_time']
            if dev_time == instance.dev_time:
                pass
            else:
                update_dict['dev_time'] = dev_time

        if 'firstboot' in body['server']:
            firstboot = body['server']['firstboot']
            if firstboot == 0:
                task_state = task_states.WAIT_POWER_ON
            elif firstboot == 2:
                task_state = task_states.WAIT_POWER_OFF
            update_dict['firstboot']= firstboot
            update_dict['task_state'] = task_state
        helpers.translate_attributes(helpers.UPDATE, server, update_dict)
        if project_id and user_id: 
            self.compute_api.allocate_resources(ctxt, instance)
    ...
    @recordinfo.error_action_log(instance_actions.RESIZE)
    ...
    @wsgi.action('batch_resize')
    def _action_batch_resize(self, req, id, body):
        """ Batch resize instance."""
        batch_resize = body.get("batch_resize", None)
        if not batch_resize:
            raise exc.HTTPUnprocessableEntity()

        context = req.environ["nova.context"]
        instance_ids = batch_resize.get("instance_ids", None)
        flavor = batch_resize.get("flavor", None)
        kwargs = {}
        helpers.translate_attributes(helpers.RESIZE, batch_resize, kwargs)
        try:
            if instance_ids and flavor:
                i = 1
    ...
    @wsgi.response(202)
    @extensions.expected_errors((400, 401, 403, 404, 409))
    @wsgi.action('audit_action')
    def _action_audit_action(self, req, id, body):
        context = req.environ['nova.context']
        instance = self._get_server(context, req, id)
        if "usbconnect" in body['audit_action'].keys():
            action_name = "Usb Connect"
            setattr(context, "message", body['audit_action']['usbconnect'])
        if "usbdisconnect" in body['audit_action'].keys():
            action_name = "Usb DisConnect"
            setattr(context, "message", body['audit_action']['usbdisconnect'])
        if "usbflowsize" in body ['audit_action'].keys():
            action_name = "Usb Flowsize"
            setattr(context, "message", body['audit_action']['usbflowsize'])
        self.action_api.action_start(context, instance, action_name)


    @wsgi.response(202)
    @extensions.expected_errors((400, 401, 403, 404, 409))
    @wsgi.action('object_info')
    def _action_control(self, req, id, body):
        context = req.environ['nova.context']
        instance = self._get_server(context, req, id)
        usb_control = instance.get('usb_control')
        clipboard_control = instance.get('clipboard_control')
        screen = instance.get('screen')
        quatity_control = instance.get('quatity_control')
        shutdown_restore = instance.get('shutdown_restore')
        multi_user = instance.get('multi_user')
        allow_screen_broadcast = instance.get('allow_screen_broadcast')
        jostle = instance.get('jostle')
        during = instance.get('during') 
        per = instance.get('per')
        dev_time = instance.get('dev_time')
        disk_type = instance.get('disk_type')
        return {'usb': usb_control, 'clipboard': clipboard_control, 
                'screen':screen, 'quatity_control':quatity_control, 
                'shutdown_restore':shutdown_restore, 'multi_user':multi_user, 
                'allow_screen_broadcast':allow_screen_broadcast, 'jostle':jostle, 
                'during':during, 'disk_type':disk_type}

    @wsgi.response(202)
    @extensions.expected_errors((400, 401, 403, 404, 409))
    @wsgi.action('usb')
    def _action_old_control(self, req, id, body):
        return self._action_control(req, id, body)

    @wsgi.response(202)
    @extensions.expected_errors((400, 401, 403, 404, 409))
    @wsgi.action('secure')
    def _action_get_spice_secure(self, req, id, body):
        context = req.environ['nova.context']
        instance = self._get_server(context, req, id)
        spice_secure = instance.get('spice_secure', False)
        return {'spice_secure': spice_secure}
    ...
    def _save_timing_info(self, req, update_dict,  data):
        ctxt = req.environ['nova.context']
        try:
            for id in data:
    ...
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('timing')
    def _action_timing_task(self, req, id, body):
        body = body.pop("timing")
        update_dict = {}
        checksplit = []

        if 'checkboxlist' in body['server']:
            checkboxlist = body['server']['checkboxlist']
            checksplit = checkboxlist.split('_')
        else:
            raise exc.HTTPException()

        if 'timing_boot' in body['server']:
            timing_boot = body['server']['timing_boot']
            if timing_boot:
                update_dict['timing_boot']=timing_boot

        if 'timing_boot_time' in body['server']:
            timing_boot_time = body['server']['timing_boot_time']
            if timing_boot_time:
                update_dict['timing_boot_time']=json.dumps(timing_boot_time)

        if 'timing_shutdown' in body['server']:
            timing_shutdown = body['server']['timing_shutdown']
            if timing_shutdown:
                update_dict['timing_shutdown']=timing_shutdown

        if 'timing_shutdown_time' in body['server']:
            timing_shutdown_time = body['server']['timing_shutdown_time']
            if timing_shutdown_time:
                update_dict['timing_shutdown_time']=json.dumps(timing_shutdown_time)
        thread.start_new_thread(self._save_timing_info,
                                   (req, update_dict, checksplit))



    @wsgi.response(202)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('os-vmpwd')
    def _get_vm_password(self, req, id, body):
        """get vm password"""
        context = req.environ['nova.context']
        instance = self._get_instance(context, id)
        cipher = instance.get('cipher')
        return {"cipher":cipher}

    @wsgi.response(202)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('remote')
    def _create_remote(self, req, id, body):
        context = req.environ['nova.context']
        value_dict ={}
        if not body.get('remote'):
            raise exc.HTTPUnprocessableEntity()
        if 'instance_id' in body['remote']:
            instance_id = body['remote']['instance_id']
            value_dict['instance_id']=instance_id
        if 'instance_name' in body['remote']:
            instance_name = body['remote']['instance_name']
            value_dict['instance_name']=instance_name
        if 'client_ip' in body['remote']:
            client_ip = body['remote']['client_ip']
            value_dict['client_ip']=client_ip
        if 'password' in body['remote']:
            password = body['remote']['password']
            value_dict['password']=password
        if 'status' in body['remote']:
            status = body['remote']['status']
            value_dict['status']= status
        if value_dict.get('instance_id', None): 
            instance_id = value_dict.get('instance_id')
            remote = self.compute_api.get_remote_assistance(context, instance_id)
            if remote:
                self.compute_api.\
                update_remote_assistance(context, instance_id, 
                                             values=value_dict)
            else:
                self.compute_api.\
                create_remote_assistance(context, value_dict)

    @wsgi.response(202)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('cancel_assistance')
    def _delete_remote(self, req, id, body):
        context = req.environ['nova.context']
        result = self.compute_api.delete_remote_assistance(context, id)

    @wsgi.response(202)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('remote_list')
    def _remote_list(self, req, id, body):
        context = req.environ['nova.context']
        instance_id = None
        if 'instance_id' in body['remote_list']:
            instance_id = body['remote_list']['instance_id']
        if instance_id:
            remote=self.compute_api.get_remote_assistance(
                   context, instance_id)
        else:
            remote=self.compute_api.\
                   remote_assistance_list(context)
