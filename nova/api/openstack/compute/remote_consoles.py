...
class RemoteConsolesController(wsgi.Controller):
    ...
    @wsgi.action('os-getPort')
    def get_port(self, req, id, body):
        context = req.environ['nova.context']
        console_type = body['os-getPort'].get('type')

        try:
            instance = common.get_instance(self.compute_api, context, id)
            output = self.compute_api.get_port(context,
                                               instance,
                                               console_type)
