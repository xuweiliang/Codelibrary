...
class ComputeManager(manager.Manager):
    ...
    @reverts_task_state
    @wrap_instance_fault
    #@recordinfo.class_action_log(instance_actions.CREATE)
...
#        if CONF.compute_driver == 'libvirt.LibvirtDriver':
#            self._build_meterage(context, instance)
        if CONF.compute_driver == 'xenapi.XenAPIDriver':
            instance.vm_hyperv_type = 'xen'
            instance.save()
    ...
    #create a meterage
    def _build_meterage(self, context, instance):
        values = {'host': instance.host,
                  'instance_id':instance.uuid}

        meterage = objects.Meterage(context=context)
        meterage.update(values)
        meterage.create()
    ...
    #@recordinfo.class_action_log(instance_actions.DELETE)
    ...
    @wrap_instance_event(prefix='compute')
    @wrap_instance_fault
    #@recordinfo.class_action_log(instance_actions.STOP)
    ...
    @wrap_instance_event(prefix='compute')
    @wrap_instance_fault
    #@recordinfo.class_action_log(instance_actions.START)
    ...
    @wrap_instance_event(prefix='compute')
    @wrap_instance_fault
    @recordinfo.class_action_log(instance_actions.RESTORE)
    ...
    @wrap_instance_event(prefix='compute')
    @wrap_instance_fault
    @recordinfo.class_action_log(instance_actions.REBOOT)
    ...
    @recordinfo.class_action_log(instance_actions.CREATESNAPSHOT)    
    ...
    @recordinfo.class_action_log(instance_actions.RESIZE)
    ...
    def cdrom_list(self, context, instance):
        cdrom = self.driver.cdrom_list(context, instance)
        return cdrom

    @wrap_exception()
    @wrap_instance_fault
    @messaging.expected_exceptions(NotImplementedError)
    @recordinfo.class_action_log(instance_actions.CDROM)
    def attach_cdroms(self, context, image_id, device, instance):
        return self.driver.attach_cdrom(context, instance, device, image_id)

    @recordinfo.class_action_log(instance_actions.CREATEDEVSNAPSHOT)
    @messaging.expected_exceptions(NotImplementedError)
    def dev_snapshot_create(self, context, instance, name):
        self.driver.dev_snapshot_create(context, instance, name)
    ...
    def dev_snapshot_list(self, context, instance):
        return  self.driver.dev_snapshot_list(context, instance)

    @recordinfo.class_action_log(instance_actions.DELETEDEVSNAPSHOT)
    @messaging.expected_exceptions(NotImplementedError)
    def dev_snapshot_delete(self, context, instance, name):
        self.driver.dev_snapshot_delete(context, instance, name)

    @messaging.expected_exceptions(NotImplementedError)
    @recordinfo.class_action_log(instance_actions.REVERTDEVSNAPSHOT)
    def dev_snapshot_revert(self, context, instance, name):
        self.driver.dev_snapshot_revert(context, instance, name)
    ...
    @wrap_exception()
    @wrap_instance_fault
    def get_port(self, context, console_type, instance):
        """Return connection information for a spice console."""
        context = context.elevated()
        LOG.debug(_("Getting port"), instance=instance)
        token = str(uuid.uuid4())
        if not CONF.spice.enabled:
            raise exception.ConsoleTypeInvalid(console_type=console_type)
    ...
    def _instance_timing_task(self, context, instance, timingTime):
        now = datetime.now()
        localtime = datetime(now.year, now.month, \
                        now.day, now.hour, now.minute)
        if timingTime == localtime:
            if (instance.vm_state == vm_states.STOPPED and 
                instance.task_state == None and instance.power_state == 4):
                instance.task_state = task_states.POWERING_ON
                instance.save(expected_task_state=[None])
                drv_state = self._get_power_state(context, instance)
                self._power_on(context, instance)
                instance.power_state = drv_state
                instance.vm_state = vm_states.ACTIVE
                instance.task_state = None
                instance.save()
            elif (instance.vm_state == vm_states.ACTIVE and
                instance.task_state == None and instance.power_state == 1):
                instance.task_state = task_states.POWERING_OFF
                instance.progress = 0
                instance.save(expected_task_state=[None])
                self._power_off_instance(context, instance, True)
                current_power_state = self._get_power_state(context, instance)
                instance.power_state = current_power_state
                instance.vm_state = vm_states.STOPPED
                instance.task_state = None
    ...
    def _instance_timing(self, context):
        db_instances = objects.InstanceList.get_by_host(context,
                                          self.host, use_slave=True)
        for instance in db_instances:
            boot_time = eval(instance.timing_boot_time or '[]')
            shutdown_time = eval(instance.timing_shutdown_time or '[]')
            boot = instance.timing_boot
            shutdown = instance.timing_shutdown
            if isinstance(boot_time, str):
                if boot == u"timing_boot":
                    dbTime= datetime.strptime(boot_time, "%Y-%m-%dT%H:%M")
                    timingTime = datetime(dbTime.year, dbTime.month, dbTime.day,
                                          dbTime.hour, dbTime.minute)
                    self._instance_timing_task(context, instance, timingTime)
            if isinstance(shutdown_time, str):
                if shutdown == u"timing_shutdown":
                    dbTime = datetime.strptime(shutdown_time, "%Y-%m-%dT%H:%M")
                    timingTime = datetime(dbTime.year, dbTime.month, dbTime.day,
                                          dbTime.hour, dbTime.minute)
                    self._instance_timing_task(context, instance, timingTime) 
            if isinstance(boot_time, dict):
                now = datetime.now()
                today_week = str(now.weekday())
                weeklist = boot_time['weeklist'].split('-')
                if today_week in weeklist:
                    w_time = datetime.strptime(boot_time['loop'], "%H:%M")
                    weektime = datetime(now.year, now.month,\
                                        now.day, w_time.hour, w_time.minute)
                    if boot == u"timing_boot":
                        self._instance_timing_task(context, instance, weektime)
            if isinstance(shutdown_time, dict):
                now = datetime.now()
                today_week = str(now.weekday())
                weeklist = shutdown_time['weeklist'].split('-')
                if today_week in weeklist:
                    w_time = datetime.strptime(shutdown_time['loop'], "%H:%M")
                    weektime = datetime(now.year, now.month,\
                                        now.day, w_time.hour, w_time.minute)
                    if shutdown == u"timing_shutdown":
                        self._instance_timing_task(context, instance, weektime)
