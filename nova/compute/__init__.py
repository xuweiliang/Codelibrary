...
def LicenceAPI(*args, **kwargs):
    """Returns the 'LicenceAPI' class from the same module as the
    configured compute api.
    """
    compute_api_class_name = _get_compute_api_class_name()
    compute_api_class = importutils.import_class(compute_api_class_name)
    class_name = compute_api_class.__module__ + ".LicenceAPI"
    return importutils.import_object(class_name, *args, **kwargs)

def RemoteAPI(*args, **kwargs):
    """Returns the 'LicenceAPI' class from the same module as the
    configured compute api.
    """
    compute_api_class_name = _get_compute_api_class_name()
    compute_api_class = importutils.import_class(compute_api_class_name)
    class_name = compute_api_class.__module__ + ".RemoteAPI"
    return importutils.import_object(class_name, *args, **kwargs)
