...
class API(base.Base):
    ...
    def _validate_and_build_base_options(self, context, instance_type,
                                         ...
                                         cipher, terminal, display_description, key_name,
        ...
        base_options = {
            ...
            'time_zone': terminal.get('time_zone', None),
            'screen': terminal.get('screen', None),
            'image_format':terminal.get('image_format', None),
            'disk_type':terminal.get('disk_type', None),
            'usb_control':terminal.get('usb_control', None),
            'clipboard_control':terminal.get('clipboard_control', None),
            'quatity_control':terminal.get('quatity_control', None),
            'shutdown_restore':terminal.get('shutdown_restore', None),
            'multi_user':terminal.get('multi_user', None),
            'allow_screen_broadcast':terminal.get('allow_screen_broadcast', None),
            'jostle':terminal.get('jostle', None),
            'during':terminal.get('during', None),
            'per':terminal.get('per', None),
            'dev_time':terminal.get('dev_time', None),
            'spice_secure':terminal.get('spice_secure', None),
            'cipher': cipher,
            'vm_hyperv_type':'QEMU',
    ...
    def _provision_instances(self, context, instance_type, min_count,
        ...
        _random = importutils.import_module('random')
        instances = []
        instance_mappings = []
        build_requests = []
        try:
            for i in range(num_instances):
                ...
                instance.cipher = _random.randint(1,99999)
    ...
    def _create_instance(self, context, instance_type,
               ...
               min_count, max_count, display_name, 
               cipher, terminal, pool, display_description,
        ...
        if terminal and 'vm_user' in terminal:
            context.user_id = terminal['vm_user']
        else:
            context.user_id = [str(uuid.uuid4()).replace('-','') 
                              if pool != context.project_id else context.user_id][0]
            context.project_id = pool or context.project_id
        ...
        base_options, max_net_count, key_pair = \
                self._validate_and_build_base_options(
                    ...
                    kernel_id, ramdisk_id, display_name, 
                    cipher, terminal, display_description,
    ...
    def create(self, context, instance_type,
               image_href, kernel_id=None, ramdisk_id=None, terminal=None,
               min_count=None, max_count=None, display_name=None, 
               cipher=None, pool=None, display_description=None,
    ...
    @recordinfo.conductor_action_log(instance_actions.DELETE)
    ...
    @recordinfo.class_action_log(instance_actions.REALLOCATE)
    def allocate_resources(self, context, instance, project_id=None, user_id=None):
        quotas = objects.Quotas(context=context)
        flavor = instance.get_flavor()
        old_project_id, old_user_id = quotas_obj.ids_from_instance(context, instance)
        instance_vcpus = instance.vcpus
        instance_memory_mb = instance.memory_mb
        if project_id or user_id:
            project_quotas = self.db.quota_usage_get_all_by_project(context, project_id)
            quota_usages = self.db.quota_usage_get_all_by_project_and_user(context, 
                                                                project_id, user_id)
           # import pdb
           # pdb.set_trace()
           # if project_quotas and quota_usages:
           #     num_instances, quotas = self._check_num_instances_quota(
           #             context, flavor, 1, 1,
           #             project_id=project_id, user_id=user_id)
           # else:
            ...
            quotas.reserve(project_id=old_project_id,
                              user_id=old_user_id, instances=-1, 
                              cores=-instance_vcpus,
                              ram =-instance_memory_mb)
        quotas.commit()
    ...
    def update_instance(self, context, instance, updates):
        ...
        #project_id, user_id = quotas_obj.ids_from_instance(
        #        context, instance)
        #LOG.info("project_id:%s ============user_id:%s" % (project_id, user_id)) 
    ...
    def cdrom_list(self, context, instance):
        return self.compute_rpcapi.cdrom_list(context, instance=instance)

    @check_instance_host
    def attach_cdrom(self, context, instance, device, image_id):
        return self.compute_rpcapi.attach_cdrom(context,
                                      instance=instance,
                                          device=device,
                                      image_id=image_id)
    ...
    def dev_snapshot_create(self, context, instance, name):
        dev_snapshot = self.compute_rpcapi.dev_snapshot_create(context, instance=instance, name = name)
        #self._record_action_start(context, instance, instance_actions.CREATEDEVSNAPSHOT)
        return dev_snapshot

    @check_instance_host
    def dev_snapshot_list(self, context, instance):
        dev_snapshots = self.compute_rpcapi.dev_snapshot_list(context, instance=instance)
        return dev_snapshots

    @check_instance_host
    def dev_snapshot_revert(self, context, instance, name):
        dev_snapshots = self.compute_rpcapi.dev_snapshot_revert(context, instance=instance, name=name)
        #self._record_action_start(context, instance, instance_actions.REVERTDEVSNAPSHOT)
        return dev_snapshots

    @check_instance_host
    def dev_snapshot_delete(self, context, instance, name):
        dev_snapshots = self.compute_rpcapi.dev_snapshot_delete(context, instance=instance, name = name)
        #self._record_action_start(context, instance, instance_actions.DELETEDEVSNAPSHOT)
        return dev_snapshots
    ...
    @check_instance_host
    def get_port(self, context, instance, console_type):
        """Get a url to an instance Console."""
        connect_info = self.compute_rpcapi.get_port(context,
                       instance=instance, console_type=console_type)
        return connect_info
    ...
    def create_remote_assistance(self, context, values):
        return objects.RemoteAssistance.\
               create_remote_assistance(context, self.session(), values=values)

    def get_remote_assistance(self, context, instance_id):
        return objects.RemoteAssistance.\
               get_remote_assistance(context, instance_id, self.session())

    def delete_remote_assistance(self, context, instance_id):
        return self.db.delete_remote_assistance(context, instance_id, self.session())
        #return objects.RemoteAssistance.\
        #       delete_remote_assistance(context, instance_id, self.session)

    def update_remote_assistance(self, context, instance_id, **kwargs):
        return objects.RemoteAssistance.\
               update_remote_assistance(context, instance_id, self.session(), **kwargs)

    def remote_assistance_list(self, context):
        return objects.RemoteAssistanceList.\
               remote_assistance_list(context, self.session())
...
class InstanceActionAPI(base.Base):
    ...
    def action_start(self, context, instance, action_name):
        return objects.InstanceAction.action_start(\
            context, instance['uuid'], action_name)
...
class LicenceAPI(base.Base):
    """Sub-set of the Compute Manager API for managing licence"""
    def get_licence_by_id(self, context):
        return objects.Licence.get_by_licence_id(context, self.session())

    def update_licence(self, context, **kwargs):
        return objects.Licence.save(context, self.session(), **kwargs)
...
class AggregateAPI(base.Base):
    ...
    @wrap_exception()
    @recordinfo.class_action_log(instance_actions.CREATEAGGREGATE)
    ...
    @wrap_exception()
    @recordinfo.class_action_log(instance_actions.UPDATEAGGREGATE)
    ...
    def delete_aggregate(self, context, aggregate_id):
        ...
        if len(aggregate.hosts) > 0:
            ...
            recordinfo.error_current_record_from_exc(context, aggregate.name,
                                                 instance_actions.DELETEAGGREGATE,
                                                 msg)
            raise exception.InvalidAggregateActionDelete(
                aggregate_id=aggregate_id, reason=msg)
        aggregate.destroy()
        recordinfo.add_current_record_for_success(context, aggregate.name,
                                              instance_actions.DELETEAGGREGATE)
...
class SystemLogsAPI(base.Base):
    def _reformat_systemlogs_info(self, systemlogs):
        return dict(systemlogs.iteritems())
    def systemlogs_create(self, context, name, event_subject, result, detail):
        if result:
            systemlogs = recordinfo.add_current_record_for_success(context, name, event_subject, detail)
        else:
            systemlogs = recordinfo.error_current_record_from_exc(context, name, event_subject, detail)
        return self._reformat_systemlogs_info(systemlogs)

    def systemlogs_list(self, context, filters=None):
        systemlogs = objects.SystemlogsList.systemlogs_list(context, self.session(), filters=filters)
        return [self._reformat_systemlogs_info(log) for log in systemlogs]
