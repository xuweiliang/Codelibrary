...
#            image_info['image_id'] = image_id
    ...
    def rescue(self, context, instance, network_info, image_meta,
        ...
        self._create_image(context, instance, disk_info['mapping'],
                           ...
                           network_info=network_info,
                           admin_pass=rescue_password,image_meta=image_meta) 
        xml = self._get_guest_xml(context, instance, network_info, disk_info,
    ...
    def _inject_data(self, injection_image, instance, network_info,
        ...
        image_type=instance.image_format
    ...
    def _create_image(self, context, instance,
                      ...
                      fallback_from_host=None, image_meta=None):
        booted_from_volume = self._is_booted_from_volume(
            instance, disk_mapping)

        #def image(fname, image_type=CONF.libvirt.images_type):
        def image(fname, image_type=instance.image_format):
            if not instance.image_format:
    ...
    def _create_and_inject_local_root(self, context, instance,
        ...
        if not booted_from_volume:
            ...
            self._try_fetch_image_cache(backend, fetch_func, context,
                                        ...
                                        instance, size, disk_format, fallback_from_host)
                                        #instance, size, fallback_from_host,disk_format=disk_format)
    ...
    def _set_clock(self, guest, instance, image_meta, virt_type):
        ...
        #else:
        #    clk.offset = 'utc'
        #migrate by  wengshua time_zone
        if instance['time_zone'] == 'localtime':
        ...
        if virt_type == "kvm":
            self._set_kvm_timers(clk, instance.os_type, image_meta)
    ...
    def _set_features(self, guest, instance, caps, virt_type):
        if virt_type == "xen":
        ...
        if virt_type in ("qemu", "kvm"):
            if instance['shutdown_restore'] == "yes":
                qemu_commandline = vconfig.LibvirtConfigGuestQemu()
                qemu_commandline.shutdown_restore = True
                guest.shutdown_restore = qemu_commandline
    ...
    def _configure_guest_by_virt_type(self, guest, virt_type, caps, instance,
        ...
        if virt_type == "xen":
            ...
            guest.os_bootmenu = vconfig.LibvirtConfigGuestBootMenu() 
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        self._set_features(guest, instance, caps, virt_type)
        #self._set_features(guest, instance.os_type, caps, virt_type)
        #self._set_clock(guest, instance.os_type, image_meta, virt_type)
        self._set_clock(guest, instance, image_meta, virt_type)
        ...
        if self.has_cdrom(instance,disk_info) is None:
            ...
            def is_iso_image_active(context, fake_image_id):
                ...
                if flug:
                    ...
                    return True
            try:
                exist_cdroms = self.cdrom_list(context, instance)
                found_instance = True
            except:
        ...
        if instance['usb_control']==True:
            ich9_ehci1 = vconfig.LibvirtConfigGuestController()
            ich9_ehci1.type = "usb"
            ich9_ehci1.model = "ich9-ehci1"
            guest.add_device(ich9_ehci1)

            ich9_uhci1 = vconfig.LibvirtConfigGuestController()
            ich9_uhci1.type = "usb"
            ich9_uhci1.model = "ich9-uhci1"
            guest.add_device(ich9_uhci1)

            ich9_uhci2 = vconfig.LibvirtConfigGuestController()
            ich9_uhci2.type = "usb"
            ich9_uhci2.model = "ich9-uhci2"
            guest.add_device(ich9_uhci2)

            ich9_uhci3 = vconfig.LibvirtConfigGuestController()
            ich9_uhci3.type = "usb"
            ich9_uhci3.model = "ich9-uhci3"
            guest.add_device(ich9_uhci3)
        ...
        if (CONF.spice.enabled and
            ...
            graphics = vconfig.LibvirtConfigGuestGraphics()
            graphics.type = "spice"
            if instance['multi_user']:
                graphics.clients= instance['multi_user']
            if instance['jostle'] == True:
                graphics.jostle = True
            if instance['quatity_control'] == True :
                graphics.quatiy= True
            if instance['clipboard_control'] == True:
                graphics.copypaste = 'yes'
            else:
                graphics.copypaste = 'no'
            graphics.passwd = "%s" % instance.get("cipher", "00000")
        ...
        if add_video_driver:
            if graphics.type == "spice":
                if instance['screen']==1:
                    self._add_video_driver(guest, image_meta, flavor)
                elif instance['screen']==2:
                    self._add_video_driver(guest, image_meta, flavor)
                    self._add_video_driver(guest, image_meta, flavor)
                elif instance['screen']==4:
                    self._add_video_driver(guest, image_meta, flavor)
                    self._add_video_driver(guest, image_meta, flavor)
                    self._add_video_driver(guest, image_meta, flavor)
                    self._add_video_driver(guest, image_meta, flavor)
        ...
        sound.model_type = "ac97"
        guest.add_device(sound)
        #add usbredir device 
        if instance['usb_control']==True:
            redir0 = vconfig.LibvirtConfigGuestRedirdev()
            redir0.name = "redir0"
            guest.add_device(redir0)

            redir1 = vconfig.LibvirtConfigGuestRedirdev()
            redir1.name = "redir1"
            guest.add_device(redir1)

            redir2 = vconfig.LibvirtConfigGuestRedirdev()
            redir2.name = "redir2"
            guest.add_device(redir2)

            redir3 = vconfig.LibvirtConfigGuestRedirdev()
            redir3.name = "redir3"
            guest.add_device(redir3)
    ...
    def _try_fetch_image_cache(self, image, fetch_func, context, filename,
                               image_id, instance, size, disk_format=None,
                               fallback_from_host=None):
        try:
            image.cache(fetch_func=fetch_func,
                        ...
                        filename=filename,
                        image_id=image_id,
                        size=size, disk_format=disk_format)
        except exception.ImageNotFound:
            if not fallback_from_host:
                raise
    ...
    def _get_instance_disk_info(self, instance_name, xml,
        ...
        for cnt, path_node in enumerate(path_nodes):
            ...
            device = disk_nodes[cnt].get('device')
            if device == 'cdrom':
                continue
    ...
    def finish_migration(self, context, migration, instance, disk_info,
        ...
        self._create_image(context, instance, block_disk_info['mapping'],
                           ...
                           fallback_from_host=migration.source_compute,image_meta=image_meta) 
