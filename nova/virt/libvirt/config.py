...
class LibvirtConfigGuestQemu(LibvirtConfigObject):

    def __init__(self):
        super(LibvirtConfigGuestQemu, self).__init__(root_name="commandline", ns_prefix="qemu", ns_uri =QEMU_NS)

        self.shutdown_restore = None

    def format_dom(self):
        qemu_commandline = super(LibvirtConfigGuestQemu, self).format_dom()
        if self.shutdown_restore:
            shutdown_restore = self._new_node("arg")
            shutdown_restore.set("value", '-snapshot')
            qemu_commandline.append(shutdown_restore)
...
class LibvirtConfigGuestGraphics(LibvirtConfigGuestDevice):
    ...
    def format_dom(self):
        ...
        if self.quatiy:
            dev.append(etree.Element('image', compression='auto_glz'))
            dev.append(etree.Element('jpeg', compression='always'))
            dev.append(etree.Element('zlib', compression='never'))
        if self.copypaste:
            dev.append(etree.Element('clipboard', copypaste=self.copypaste))
        if self.mouse:
            dev.append(etree.Element('mouse', mode='server'))
        if self.clients:
            dev.append(etree.Element('multiple', clients=self.clients))
        if self.jostle:
            dev.append(etree.Element('playback', compression='off'))
        else:
            dev.append(etree.Element('playback', compression='on'))
...
class LibvirtConfigGuest(LibvirtConfigObject):
    ...
    def __init__(self, **kwargs):
        ...
        self.idmaps = []
        self.perf_events = []
        self.os_bootmenu = None  #wengshuhua
    ...
    def _format_os(self, root):
        ...
        #wengshuhua
        if self.os_bootmenu is not None:
            os.append(self.os_bootmenu.format_dom())
    ...
    def format_dom(self):
        ...
        if self.shutdown_restore is not None:
            root.append(self.shutdown_restore.format_dom())
...
class LibvirtConfigGuestBootMenu(LibvirtConfigObject):

    def __init__(self, **kwargs):
        super(LibvirtConfigGuestBootMenu, self).__init__(root_name="bootmenu",
                                                       **kwargs)

        self.enable = "yes"

    def format_dom(self):
        bootmenu = super(LibvirtConfigGuestBootMenu, self).format_dom()
        bootmenu.set("enable", self.enable)
...
class LibvirtConfigGuestSound(LibvirtConfigGuestDevice):
    ...
    def format_dom(self):
        ...
        dev.set("model", self.model_type)

        return dev

class LibvirtConfigGuestRedirdev(LibvirtConfigGuestDevice):

    def __init__(self, **kwargs):
        super(LibvirtConfigGuestRedirdev, self).__init__(root_name="redirdev",
                                                        **kwargs)
    ...
    def format_dom(self):
        dev = super(LibvirtConfigGuestRedirdev, self).format_dom()
        dev.set("bus", self.bus)
        dev.set("type", self.type)

        if self.name is not None:
            alias = etree.Element("alias", name=self.name)
            dev.append(alias)
