...
class SchedulerManager(manager.Manager):
    ...
    @periodic_task.periodic_task(spacing=CONF.scheduler_driver_task_period,
                                 run_immediately=True)
    def _run_periodic_licence(self, context):
        Licence = objects.Licence()
        licence = Licence.get_by_licence_id(context, db.base.Session())
        starttime = licence.starttime
        time_stamp = time.mktime(starttime.timetuple())
        sys_stamp = time.mktime(licence.updated_at.timetuple())
        count_time = datetime.fromtimestamp(time_stamp+CONF.scheduler_driver_task_period)
        sys_time =datetime.fromtimestamp(sys_stamp)
        if sys_stamp < time_stamp:
            Licence.save(context, db.base.Session(), 
            starttime=datetime.strftime(count_time,'%Y-%m-%d %H:%M:%S'))
        else:
            Licence.save(context, db.base.Session(), 
            starttime=datetime.strftime(sys_time, '%Y-%m-%d %H:%M:%S'))
