...
def get_engine(context):
    return IMPL.get_engine(context)
...
def get_licence(context, id, session=None):
    """licence get by id, Call current interface not required session parame
       param id default 1
       param: session=session or None
       return: licence object""" 
    return IMPL.get_licence(context, id, session=session)


def update_licence(context, id, session=None, **kwargs):
    """licence by id Re registration, 
       Call current interface not required session parame"""
    return IMPL.update_licence(context, id, session=session, **kwargs)

def create_remote(context, *argws, **kwargs):
    """Create a remote assistance request""" 
    return IMPL.create_remote(context, *argws, **kwargs)

def get_remote_assistance(context, instance_id, session):
    """get a remote assistance by instance_id"""
    return IMPL.get_remote_assistance(context, instance_id, session)

def delete_remote_assistance(context, instance_id, session):
    """delete a remote assistance by instance_id"""
    return IMPL.delete_remote_assistance(context, instance_id, session)

def update_remote_assistance(context, *args, **kwargs):
    """update a remote assistance by instance_id""" 
    return IMPL.update_remote_assistance(context, *args, **kwargs)

def remote_assistance_list(context, session):
    """get all remote assistance"""
    return IMPL.remote_assistance_list(context, session)

def systemlogs_create(context, logsinfo):
    return IMPL.systemlogs_create(context, logsinfo)

def systemlogs_list(context, session, filters=None):
    return IMPL.systemlogs_list(context, session, filters)
