...
####################
def get_used(context, session=None):

    try:
        query = model_query(context, models.Instance, 
                read_deleted="no").count()
        if not query:
            raise Exception()
        return query
    except Exception:
        if session:
            ...
            return query.filter(
                   models.Instance.deleted == 'no').count()


def get_licence(context, id, session=None):

    try:
        query = model_query(context, models.Licence,
              read_deleted="yes").filter_by(id=id).first()
        if not query:
            raise Exception()
        return query
    except Exception:
        if session:
            session.bind=get_engine(context)
            query = session.query(models.Licence).\
            filter_by(id=id).first()
            query.used = get_used(context, session=session)
            return query
            

def update_licence(context, id, session=None, **kwargs):
...
def create_remote(context, session, **kwargs):
    remote_ref = models.RemoteAssistance()
    remote_ref.update(kwargs['values'])
    engine = get_engine(context)
    session.bind=engine
    session.add(remote_ref)
    session.flush()
    session.commit()

def get_remote_assistance(context, instance_id, session):

    if session:
        try:
            session.bind=get_engine(context)
            query = session.query(models.RemoteAssistance).\
...
def delete_remote_assistance(context, instance_id, session):
    if session:
        try:
            remote = get_remote_assistance(context, 
                     instance_id, session)
            if remote:
                session.bind=get_engine(context)
                query = session.query(models.RemoteAssistance).\
                filter_by(id=remote.id).delete()
                session.flush()
                session.commit()
        except Exception:
            raise ValueError(_("No session cannot connect to the database"))
        

def update_remote_assistance(context, instance_id, session, **kwargs):
    if session:
        session.bind=get_engine(context)
        query = session.query(models.RemoteAssistance).\
        filter_by(instance_id=instance_id).update(kwargs['values'])
        session.flush()
        session.commit()

def remote_assistance_list(context, session):
    if session:
        try:
            session.bind=get_engine(context)
            query = session.query(models.RemoteAssistance).all()
            session.flush()
            session.commit()
            return query
        except Exception:
...
def systemlogs_create(context, logsinfo):
    systemlogs_ref = models.Systemlog()
    logsinfo['id'] = str(uuid.uuid4())
    systemlogs_ref.update(logsinfo)
    query = context.Session.query(models.Systemlog)
    if len(query.all())>(1000-1):
        for i in range(len(query.all())-(1000+1)):
            first_id = query.first().id
            query.filter_by(id=first_id).delete()
    context.Session.add(systemlogs_ref)
    context.Session.flush()
    context.Session.commit()
    return systemlogs_ref

def systemlogs_list(context, session, filters=None):
#    import pdb
#    pdb.set_trace()
    session.bind=get_engine(context)
    query =session.query(models.Systemlog)
#    try:
    if filters is not None:
        search = filters.split("-")
        param = search[0]
        item = search[1]
        if item == "user_name":
            result = query.filter_by(user_name=param).order_by(desc("created_at"))
        elif item == "project_name":
            result = query.filter_by(project_name=param).order_by(desc("created_at"))
        elif item == "result":
            result = query.filter_by(result=param).order_by(desc("created_at"))
        elif item == "time":
            result = query.order_by(desc("created_at"))
        elif item == "visit_ip":
            result = query.filter_by(visit_ip=param).order_by(desc("created_at"))
    else:
        result = query.order_by(desc("created_at"))
    return result.all()
#    except Exception:
