...
class Instance(BASE, NovaBase, models.SoftDeleteMixin):
    ...
    dev_snapshot = Column(String(255))
    vm_hyperv_type = Column(String(39), default='QEMU')
    ...
    # New features
    # New secondary personal name database instances
    cipher = Column(Integer)
    timing_boot = Column(String(255))
    timing_boot_time = Column(String(255))
    timing_shutdown = Column(String(255))
    timing_shutdown_time = Column(String(255))
    time_zone= Column(String(39))
    client_ip = Column(String(255))
    client_time = Column(String(255))
    screen= Column(Integer)
    image_format= Column(String(39))
    disk_type= Column(String(39))
    usb_control= Column(Boolean(),default=True)
    clipboard_control = Column(Boolean(),default=True)
    quatity_control= Column(Boolean(),default=True)
    shutdown_restore = Column(String(39))
    multi_user = Column(String(39))
    allow_screen_broadcast = Column(Boolean(),default=False)
    jostle = Column(Boolean(),default=True)
    during = Column(String(255))
    per = Column(String(255))
    dev_time = Column(String(255))
    firstboot = Column(Integer, default=1)
    spice_secure = Column(Boolean(),default=False)



    # NOTE(jdillaman): locked deprecated in favor of locked_by,
	# vm_user':{'type':'string','format':'uuid'}
...
# NOTE(alaski): This table exists in the nova_api database and its usage here
# is deprecated.
class Licence(BASE, NovaBase):
    """Represents a licence of customized extension."""
    __tablename__ = 'licence'
    __table_args__ = ()
    id = Column(Integer, primary_key=True)
    starttime = Column(DateTime, nullable=False)
    system_uuid = Column(String(255),nullable=False)
    encrypted_license = Column(String(255),nullable=False)
    disabled = Column(Boolean, default=False)

class RemoteAssistance(BASE, NovaBase):
    __tablename__='remote_assistance'
    __table_args__=()
    id = Column(Integer, primary_key=True, nullable=True, autoincrement=True)
    instance_id = Column(String(36), nullable=True)
    client_ip = Column(String(36), nullable=True)
    instance_name=Column(String(36), nullable=False)
    password = Column(Integer, nullable=False)
    status = Column(String(36), nullable=False)
    deleted = Column(Integer, default=0)


class Systemlog(BASE, NovaBase):
    __tablename__ = 'systemlogs'
    __table_args__ = ()
    id = Column(String(36), primary_key=True, nullable=True, autoincrement=True)
    event_subject = Column(String(255), nullable=False)
    event_object = Column(String(255), nullable=False)
    user_name = Column(String(255), nullable=False)
    project_name = Column(String(255), nullable=False)
    visit_ip = Column(String(36), nullable=False)
