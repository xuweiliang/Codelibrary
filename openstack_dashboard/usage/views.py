...
class UsageView(tables.DataTableView):
    ...
    def get_data(self):
        try:
            ...
            #LOG.info("usage ============================%s" % self.usage.limits)
            #LOG.info("usage ============================%s" % self.usage.show_deleted)
            #LOG.info("csv ============================%s" % self.csv_template_name)
    ...
    def get_context_data(self, **kwargs):
        ...
        used = self.usage.get_instances()
        ins_resource = self.usage.get_resource(self.usage.limits, used=len(used))
        if ins_resource:
            self.usage.limits.update(ins_resource)
#        LOG.info("test ======================%s" % self.template_name)
            
        for t in types:
            LOG.info("tttttttttttttttttttt %s %s", t[0], t[1])
