...
class TerminalControlAction(workflows.Action):
    ...
    image_format = forms.ChoiceField(label =_("Disk Mode"),
                                   ...
                                   help_text=_("The default disk mode of virtual \
                                               machine is thin provision,if you want to create\
                                               a real-mode,please change the drop-down box."))
    ...
    class Meta:
        ...
        help_text_template= ("admin/instances/_launch_instance_terminal_help.html")
...
class SetInstanceDetailsAction(workflows.Action):
    ...
    input_user = forms.CharField(label=_("Current user"),
                            required=False,
                            help_text=_("Virtual machine is created in the current user,"
    ...
    vol_delete_on_instance_delete = forms.BooleanField(
        ...
        initial=False,
        required=False,
        help_text=_("Delete volume when \
    ...
    def __init__(self, request, context, *args, **kwargs):
        ...
        self.fields['input_user'].initial = request.user.username
    ...
    def _check_source_custom_id(self, cleaned_data):
#            project_name = cleaned_data.get('pool')
#            if not cleaned_data.get('pool'):
#                project_name= self.request.user.project_name
#            tenant_type=self.context['tenant_type'].get(project_name, None)
#            if cleaned_data.get('pool') and cleaned_data.get('input_user'):
#                if tenant_type == 'float':
#                    msg = _("Floating pool cannot be specified user,\
#                             please select other create mode.")
#                    self._errors['pool'] = self.error_class([msg])
            if not cleaned_data.get('image_snapshot_id'):
                msg = _("You must select an image.")
                self._errors['image_snapshot_id'] = self.error_class([msg])
            if not cleaned_data.get('name'):
                msg = _("This field is required.")
                self._errors['name'] = self.error_class([msg])
            if cleaned_data.get('pool') and \
                cleaned_data.get('input_user') == unicode(self.request.user):
                msg = _("Users with admin permissions cannot be \
                        specified Will lead to unable to log in.")
                self._errors['input_user'] = self.error_class([msg])
        ...
        if not cleaned_data.get('templet_id'):
            ...
            try:
                ...
                if count >= cleaned_data.get('num', 0):
                    ...
                    self._errors['templet_id'] = self.error_class([msg])
                cleaned_data.update({'templet':templet}) 
            except Exception:
    ...
    def _check_source(self, cleaned_data):
        ...
        source_check_methods = {
            #'custom_id': self._check_source_custom_id,
    ...
    def clean(self):
        ...
        cleaned_data.update({'users':users}) 
...
class SetInstanceDetails(workflows.Step):
    ...
    contributes = ("source_type", "source_id",
                   ...
                   "device_name", "pool", "input_user",# Can be None for an image.
...
class LaunchInstance(workflows.Workflow):
    ...
    def batch_handle(self, request, context, image_id, templet, users, 
        ...
        try:
            ...
            api.nova.systemlogs_create(request,'-',\
                record_action.CREATEINSTANCE,\
                result=True, detail=_("Create Instance Success"))
            ...
            api.nova.systemlogs_create(request,'-',\
                     record_action.CREATEINSTANCE,\
                     result=False, detail=_("Create Instance Fail"))
            return False
    ...
    def handle(self, request, context):
        ...
        if source_type in ['image_id', 'instance_snapshot_id', 'custom_id', 'templet_id']:
