...
class LaunchImage(tables.LinkAction):
    ...
    def get_link_url(self, datum):
        ...
        if datum.disk_format == "raw":
            source_type = 'templet_id'
        else:
...
#class DeleteTemplate(DeleteImage):
class DeleteTemplate(tables.DeleteAction):
    ...
    policy_rules = (("image", "delete_template"),)
...
class DeleteFile(tables.DeleteAction):
    ...
    policy_rules = (("image", "delete_file"),)
    def allowed(self, request, image=None):
        # Protected images can not be deleted.
        if image and image.protected:
            return False
...
class CreateImage(tables.LinkAction):
    ...
    url = "horizon:admin:images:images:create"
    classes = ("ajax-modal",)
    icon = "plus"
...
class UploadFile(tables.LinkAction):
    ...
    url = "horizon:admin:images:images:upload"
    classes = ("ajax-modal",)
    icon = "plus"
    policy_rules = (("file", "add_file"),)

class EditImage(tables.LinkAction):
    name = "edit"
    verbose_name = _("Edit Image")
    url = "horizon:admin:images:images:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class DownloadImage(tables.LinkAction):
    ...
    url = "horizon:admin:images:images:download"
    classes = ("ajax-modal",)
    icon = "plus"
    #policy_rules = (("file", "add_file"),)
...
class ImagesTable(tables.DataTable):
    ...
    name = tables.Column(get_image_name,
                         link=("horizon:admin:images:images:detail"),
...
#                              filters=(filters.yesno, filters.capfirst))
	...
	row_class = UpdateRow
        ...
        row_actions = (LaunchImage, CreateVolumeFromImage,
                       DownloadImage, EditImage, DeleteImage,)
        pagination_param = "image_marker"

class TemplatesTable(ImagesTable):
    name = tables.Column(get_image_name,
                         link=("horizon:admin:images:images:detail"),
	...
	row_class = UpdateRow
        ...
        row_actions = (LaunchImage, CreateVolumeFromImage,
                       DownloadImage, EditTemplate, DeleteTemplate,)
        pagination_param = "template_marker"

class UploadsTable(ImagesTable):
    name = tables.Column(get_image_name,
    ...
    class Meta:
        ...
        row_class = UpdateRow
        status_columns = ["status"]
        verbose_name = _("Upload Files")
        table_actions = (UploadFile, DeleteFile,)
        pagination_param = "upload_marker"
