...
class CreateForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            api.nova.systemlogs_create(request, data['name'],
                                   record_action.CREATEROUTER)
            return router
            ...
            api.nova.systemlogs_create(request, data['name'], 
                record_action.CREATEROUTER, result=False, detail=msg)
            return False
...
class UpdateForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            api.nova.systemlogs_create(request, data['name'],
                record_action.UPDATEROUTER, detail=msg)
            return router
        except Exception:
            msg = _('Failed to update router %s') % data['name']
            LOG.info(msg)
            api.nova.systemlogs_create(request, data['name'], 
                                  record_action.UPDATEROUTER, 
                                     result=False, detail=msg)
