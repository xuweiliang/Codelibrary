...
class AddComputeHostForm(forms.SelfHandlingForm):
    compute_server = forms.IPField(label=_("Add Compute IP Addr"))
    compute_user = forms.CharField(label=_("Compute User"),
                                   initial = "root",
                                   widget=forms.TextInput(attrs={'readonly': 'readonly'}))

    compute_root_pw = forms.CharField(label=_("password"),
                            widget=forms.PasswordInput(render_value=False))

    def clean(self):
        cleaned_data = super(AddComputeHostForm, self).clean()
        return cleaned_data


    def handle(self, request, data):
        try:
            compute = api.device.add_compute_node(request, data)
            messages.success(request, _('Compute node add successfully!'))
            return True
        except Exception:
            msg = _('Compute node add failed!')
            messages.error(request, msg)
            return HttpResponseRedirect('/dashboard/admin/aggregates')
...
class EvacuateHostForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            redirect = reverse('horizon:admin:aggregates:index')
