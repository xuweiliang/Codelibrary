...
class IncreaseVolume(tables.LinkAction):
    name = "increase"
    verbose_name = _("Extend Volume Storage")
    url = "horizon:admin:volumes:volumes:increase"
    classes = ("ajax-modal",)
    icon = "plus"
    policy_rules = (("volume", "volume:increase"),)
    ajax = True

    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(IncreaseVolume, self).__init__(attrs, **kwargs)
...
class ExtendVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:extend"
...
class EditAttachments(tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:attach"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class CreateSnapshot(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:create_snapshot"
    classes = ("ajax-modal",)
    icon = "camera"
...
class CreateBackup(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:create_backup"
    classes = ("ajax-modal",)
...
class UploadToImage(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:upload_to_image"
    classes = ("ajax-modal",)
    icon = "cloud-upload"
...
class EditVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:update"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class RetypeVolume(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:retype"
    classes = ("ajax-modal",)
    icon = "pencil"
...
class VolumesTable(VolumesTableBase):
    host = tables.Column(get_host_name, verbose_name=_("Host"))
    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:admin:volumes:volumes:detail")
...
class DetachVolume(tables.BatchAction):
    ...
    def allowed(self, request, attachment):
        if attachment is not None:
            return attachment['status'] in ("SHUTDOWN", "SHUTOFF", "CRASHED")
...
class StopInstance(tables.BatchAction):
    ...
    def allowed(self, request, attachment):
        if attachment is not None:
            return attachment['status'] in ("ACTIVE")

    def action(self, request, obj_id):
        attachment = self.table.get_object_by_id(obj_id)
        api.nova.server_stop(request, attachment.get('server_id', None))
...
class AttachmentsTable(tables.DataTable):
    ...
    class Meta:
        ...
        table_actions = (DetachVolume, StopInstance)
        row_actions = (DetachVolume, StopInstance)
