...
class AttachForm(forms.SelfHandlingForm):
    instance = forms.ChoiceField(label=_("Attach to Instance"),
                                 help_text=_("Select an instance to attach to. If disk type of instance is IDE, "
                                             "you must shutdown the instance when you want to attach to. In addition,\
                                              the instances in vGPU cannot attach volume."))
    ...
    def __init__(self, *args, **kwargs):
        ...
        for instance in instance_list:
            if instance.status in tables.VOLUME_ATTACH_READY_STATES and \
                ...
                try:
                    obj_info = api.nova.get_object_info(self.request, instance.id)
                except Exception:
                    exceptions.handle(self.request, err_msg)
                if instance.vm_hypervisor_type != 'xen':
                    if obj_info.get("disk_type") == "ide" and \
                        instance.status in tables.VOLUME_ATTACH_READY_STATE:
                        instances.append((instance.id, '%s' % (instance.name)))
                    elif obj_info.get("disk_type") != "ide":
                        instances.append((instance.id, '%s' % (instance.name)))
        if instances:
...
class ExtendForm(forms.SelfHandlingForm):
    ...
    def clean(self):
        ...
        if availableGB < (new_size - orig_size):
            message = _('Volume cannot be extended to %(req)GB as '
                        'you only have %(avail)GB of your quota '
                        'available.')
