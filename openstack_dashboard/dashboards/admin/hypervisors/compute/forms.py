...
class PlanDevsnapshotForm(forms.SelfHandlingForm):

    id  = forms.CharField(label=_("id"),
                                   widget=forms.TextInput(
                                       attrs={'readonly': 'readonly'}))

    time = forms.CharField(max_length=255, label=_("Time"))

    status = forms.BooleanField(label=_("Whether to use the plan snapshot"),
                                     required=False)

    def __init__(self, request, *args, **kwargs):
        super(PlanDevsnapshotForm, self).__init__(request, *args, **kwargs)
        initial = kwargs.get('initial', {})
        id = initial.get('id')
        try:
            dev_snapshot = api.nova.plan_dev_snapshot(request, id)
            plan_time = dev_snapshot.time
            status = dev_snapshot.status
            self.fields['time'].initial = plan_time
    ...
    def handle(self, request, data):
        try:
            api.nova.plan_dev_snapshot_update(request,
                                              data['id'],
                                              data['time'],
                                              data['status'])
        except Exception:
