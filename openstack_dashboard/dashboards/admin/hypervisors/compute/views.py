...
class PlanDevsnapshotView(forms.ModalFormView):
    form_class = project_forms.PlanDevsnapshotForm
    template_name = 'admin/hypervisors/compute/plan_dev_snapshot.html'
    context_object_name = 'compute_host'
    success_url = reverse_lazy("horizon:admin:hypervisors:index")

    def get_context_data(self, **kwargs):
        context = super(PlanDevsnapshotView, self).get_context_data(**kwargs)
        context["compute_host"] = self.kwargs['compute_host']
        return context


    def get_initial(self):
        initial = super(PlanDevsnapshotView, self).get_initial()
        id = self.kwargs['compute_host']
        initial.update({'id': id,})
