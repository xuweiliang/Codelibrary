...
from openstack_dashboard.dashboards.admin.network_topology.instances \
    import tables as instances_tables
from openstack_dashboard.dashboards.admin.network_topology.networks \
    import tables as networks_tables
from openstack_dashboard.dashboards.admin.network_topology.ports \
    import tables as ports_tables
from openstack_dashboard.dashboards.admin.network_topology.routers \
    import tables as routers_tables
from openstack_dashboard.dashboards.admin.network_topology.subnets \
    import tables as subnets_tables
from openstack_dashboard.dashboards.admin.network_topology \
    import tabs as topology_tabs
from openstack_dashboard.dashboards.admin.network_topology import utils

from openstack_dashboard.dashboards.admin.instances import\
...
from openstack_dashboard.dashboards.admin.instances import\
    views as i_views
from openstack_dashboard.dashboards.admin.instances.workflows import\
    create_instance as i_workflows
from openstack_dashboard.dashboards.admin.networks.subnets import\
    views as s_views
from openstack_dashboard.dashboards.admin.networks.subnets import\
...
from openstack_dashboard.dashboards.admin.routers.ports.tables import \
    DISPLAY_CHOICES as ports_choices
from openstack_dashboard.dashboards.admin.routers.ports.tables import \
    STATUS_DISPLAY_CHOICES as ports_status_choices
from openstack_dashboard.dashboards.admin.routers.ports import\
    views as p_views
from openstack_dashboard.dashboards.admin.routers.tables import \
    ADMIN_STATE_DISPLAY_CHOICES as routers_admin_choices
from openstack_dashboard.dashboards.admin.routers.tables import \
    STATUS_DISPLAY_CHOICES as routers_status_choices
from openstack_dashboard.dashboards.admin.routers import\
    views as r_views
...
class NTAddInterfaceView(p_views.AddInterfaceView):
    success_url = "horizon:admin:network_topology:index"
    failure_url = "horizon:admin:network_topology:index"

    def get_success_url(self):
        return reverse("horizon:admin:network_topology:index")

    def get_context_data(self, **kwargs):
        context = super(NTAddInterfaceView, self).get_context_data(**kwargs)
        context['form_url'] = 'horizon:admin:network_topology:interface'
        return context


class NTCreateRouterView(r_views.CreateView):
    template_name = 'admin/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:admin:network_topology:index")
...
class InstanceView(i_views.IndexView):
    ...
    template_name = 'admin/network_topology/iframe.html'
...
class JSONView(View):
    ...
    def _get_networks(self, request):
        ...
        for network in neutron_networks:
            ...
            self.add_resource_url('horizon:admin:networks:subnets:detail',
                                  obj['subnets'])
            networks.append(obj)
                ...
                try:
                    ...
                    for subnet in publicnet.subnets:
                        ...
                        self.add_resource_url(
                            'horizon:admin:networks:subnets:detail', snet)
                        subnets.append(snet)
                except Exception:
                    subnets = []
                networks.append({
    ...
    def _get_routers(self, request):
        ...
        self.add_resource_url('horizon:admin:routers:detail', routers)
        return routers
