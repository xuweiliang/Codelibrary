...
from openstack_dashboard.dashboards.admin.access_and_security.floating_ips \
    import workflows
from openstack_dashboard.dashboards.admin.vgpu import tabs
from openstack_dashboard.dashboards.admin.vgpu.workflows \
...
class TimingBoot(tables.LinkAction):
    name = "timingboot"
    verbose_name = _("Timing Boot")
    url = "horizon:admin:vgpu:timingboot"
    classes = ("ajax-modal", "btn-launch")
    ajax = True

    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(TimingBoot, self).__init__(attrs, **kwargs)
...
class TimingShutdown(tables.LinkAction):
    name = "timingshutdown"
    verbose_name = _("Timing Shutdown")
    url = "horizon:admin:vgpu:timingshutdown"
    classes = ("ajax-modal", "btn-launch")
    ajax = True

    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(TimingShutdown, self).__init__(attrs, **kwargs)
...
class UpdateInstanceResourceLink(tables.LinkAction):
    name = "resource"
    verbose_name = _("Resize Instance")
    url = "horizon:admin:vgpu:resource"
    classes = ("ajax-modal", "btn-launch")
    ajax = True

    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(UpdateInstanceResourceLink, self).__init__(attrs, **kwargs)
...
class RebootInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action_present(count):
        return ungettext_lazy(
            u"Hard Reboot VGPUInstance",
            u"Hard Reboot VGPUInstances",
            count
        )
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Hard Rebooted VGPUInstance",
            u"Hard Rebooted VGPUInstances",
            count
        )
...
class SoftRebootInstance(RebootInstance):
    ...
    def action_present(count):
        return ungettext_lazy(
            u"Soft Reboot VGPUInstance",
            u"Soft Reboot VGPUInstances",
            count
        )
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Soft Rebooted VGPUInstance",
            u"Soft Rebooted VGPUInstances",
            count
        )
...
class LaunchLink(tables.LinkAction):
    name = "launch"
    verbose_name = _("Launch VGPUInstance")
    url = "horizon:admin:vgpu:launch"
    classes = ("ajax-modal", "btn-launch")
    icon = "cloud-upload"
    ...
    def allowed(self, request, datum):
        try:
            ...
            licence = api.nova.get_licence(request)
            ...
            if not probation:
                ...
                rtf = datetime.datetime.strptime(licence.starttime,'%Y-%m-%dT%H:%M:%S.%f') 
            ...
            if instances_available <= 0 or cores_available <= 0 \
                ...
                if "disabled" not in self.classes:
                    ...
                    self.verbose_name = string_concat(self.verbose_name, ' ',
                                                      _("(Quota exceeded)"))
            elif instances_available > 0 or cores_available > 0 \
                     or ram_available > 0 or num > used \
                     or expiretime > runTime or licence.disabled is False:
...
class EditInstance(policy.PolicyTargetMixin, tables.LinkAction):
    name = "edit"
    verbose_name = _("Edit VGPUInstance")
    url = "horizon:admin:vgpu:update"
    classes = ("ajax-modal",)
    icon = "pencil"
    policy_rules = (("compute", "compute:update"),)

    def get_link_url(self, admin):
        return self._get_link_url(admin, 'instance_info')

    def _get_link_url(self, admin, step_slug):
        base_url = urlresolvers.reverse(self.url, args=[admin.id])
...
class EditInstanceSecurityGroups(EditInstance):
    ...
    def get_link_url(self, admin):
        return self._get_link_url(admin, 'update_security_groups')
...
class CreateSnapshot(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:images:snapshots:create"
    classes = ("ajax-modal",)
    icon = "camera"
...
class ResizeLink(policy.PolicyTargetMixin, tables.LinkAction):
    name = "resize"
    verbose_name = _("Resize VGPUInstance")
    url = "horizon:admin:vgpu:resize"
    classes = ("ajax-modal", "btn-resize")
    policy_rules = (("compute", "compute:resize"),)

    def get_link_url(self, admin):
        return self._get_link_url(admin, 'flavor_choice')
...
class RebuildInstance(policy.PolicyTargetMixin, tables.LinkAction):
    name = "rebuild"
    verbose_name = _("Rebuild VGPUInstance")
    classes = ("btn-rebuild", "ajax-modal")
    url = "horizon:admin:vgpu:rebuild"
...
class DecryptInstancePassword(tables.LinkAction):
    ...
    url = "horizon:admin:vgpu:decryptpassword"
...
class AssociateIP(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:access_and_security:floating_ips:associate"
    classes = ("ajax-modal",)
    icon = "link"
...
class StartInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action_present(count):
        return ungettext_lazy(
            u"Start VGPUInstance",
            u"Start VGPUInstances",
            count
        )
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Started VGPUInstance",
            u"Started VGPUInstances",
            count
        )
...
class StopInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def action_present(count):
        return npgettext_lazy(
            ...
            u"Shut Off VGPUInstance",
            u"Shut Off VGPUInstances",
            count
        )
...
class ReallocationInstance(tables.LinkAction):
    name = "reallocation"
    verbose_name = _("Reallocation Instance")
    url = "horizon:admin:vgpu:reallocation"
    classes = ("ajax-modal",)
...
class MigrateInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    data_type_singular = _("VGPUInstance")
    data_type_plural = _("VGPUInstances")
    classes = ("btn-migrate")
...
class LiveMigrateInstance(policy.PolicyTargetMixin,
                          tables.LinkAction):
    name = "live_migrate"
    verbose_name = _("Live Migrate VGPUInstance")
    url = "horizon:admin:vgpu:live_migrate"
...
class EditCDRom(policy.PolicyTargetMixin, tables.LinkAction):
    name = "CDRom"
    verbose_name = _("Edit CDrom")
    url = "horizon:admin:vgpu:editcdrom"
    classes = ("ajax-modal",)
    icon = "camera"
#    policy_rules = (("compute", "compute:editcdrom"),)
...
class AttachVolume(tables.LinkAction):
    ...
    url = "horizon:admin:vgpu:attach_volume"
    classes = ("ajax-modal",)
...
class DetachVolume(AttachVolume):
    ...
    url = "horizon:admin:vgpu:detach_volume"
...
class AttachInterface(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:vgpu:attach_interface"
...
class DetachInterface(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    classes = ("btn-confirm", "ajax-modal")
    url = "horizon:admin:vgpu:detach_interface"
...
class EditTimingBoot(policy.PolicyTargetMixin, tables.LinkAction):
    name = "settimingboot"
    verbose_name = _("Timing Boot")
    url = "horizon:admin:vgpu:settimingboot"
    classes = ("ajax-modal",)
    icon = "pencil"
    policy_rules = (("compute","compute:settimingboot"),)

    def get_link_url(self, project):
        return self._get_link_url(project, 'single_set')

    def _get_link_url(self, project, step_slug):
        base_url = urlresolvers.reverse(self.url, args=[project.id])
        next_url = self.table.get_full_url()
        params = {"step": step_slug,
                  edit_timing.SingleTimingBoot.redirect_param_name:next_url}
        param = urlencode(params)
        return "?".join([base_url, param])


class EditTimingShutdown(policy.PolicyTargetMixin, tables.LinkAction):
    name = "settimingshutdown"
    verbose_name = _("Timing Shutdown")
    url = "horizon:admin:vgpu:settimingshutdown"
    classes = ("ajax-modal",)
    icon = "pencil"
    policy_rules = (("compute","compute:settimingshutdown"),)
    ...
    def _get_link_url(self, project, step_slug):
        ...
        next_url = self.table.get_full_url()
        params = {"step": step_slug,
                  edit_timing.SingleTimingShutdown.redirect_param_name:next_url}
        param = urlencode(params)
        return "?".join([base_url, param])


def get_ips(instance):
    template_name = 'admin/vgpu/_instance_ips.html'
...
def get_vcpus(instance):
    #LOG.info("vcpus ==================%s", instance)
    if hasattr(instance, "full_flavor"):
        return instance.full_flavor.vcpus
    return _("Not Available CPU")

def get_ram(instance):
    if hasattr(instance, "full_flavor"):
        return instance.full_flavor.ram
    return _("Not Available Ram")

def get_size(instance):
    if hasattr(instance, "full_flavor"):
        template_name = 'admin/vgpu/_instance_flavor.html'
        ...
        if instance.full_flavor.disk > 0:
            ...
            size_disk = _("%s GB") % "0"
        if instance.full_flavor.ephemeral > 0:
            size_ephemeral = sizeformat.diskgbformat(instance.full_flavor.ephemeral)
        else:
            size_ephemeral = _("%s GB") % "0"
#        context = {
#            "name": instance.full_flavor.name,
#            "id": instance.id,
#            "size_disk": size_disk,
#            "size_ram": size_ram,
#            "size_ephemeral": size_ephemeral,
...
class InstancesFilterAction(tables.FilterAction):
    ...
    def filter(self, table, instance, filter_string):
        """Naive case-insensitive search."""
        q = filter_string.lower()
        return [inst_name for inst_name in instance
                if q in inst_name.name.lower()]
#    filter_type = "server"
...
class InstancesTable(tables.DataTable):
    ...
    name = tables.WrappingColumn("name",
                                 link="horizon:admin:vgpu:detail",
    ...
    vcpu = tables.Column(get_vcpus, verbose_name=_("CPU"))
    ram = tables.Column(get_ram, verbose_name=_("RAM (MB)"))

    az = tables.Column("OS-EXT-SRV-ATTR:host",
                       verbose_name=_("Host"))
...
#                            attrs={'data-type': 'timesince'})
    ...
    class Meta(object):
        ...
        row_class = UpdateRow
        prev_marker = "prev_pagination_param"
        marker = "pagination_param"
        #table_actions_menu = (StartInstance, StopInstance, SoftRebootInstance, TimingBoot, TimingShutdown)
        launch_actions = ()
        ...
        table_actions = launch_actions +(UpdateInstanceResourceLink,
                        TimingBoot,TimingShutdown, StartInstance, 
                        StopInstance, SoftRebootInstance,)+\
                        (DeleteInstance, InstancesFilterAction)
        row_actions = (StartInstance, ConfirmResize, RevertResize, 
                       ReallocationInstance, 
                       #MigrateInstance, LiveMigrateInstance, 
                       CreateSnapshot, 
                       #SimpleAssociateIP, AssociateIP, 
                       #SimpleDisassociateIP, 
                       #EditInstance, 
                       #EditCDRom,
                       EditTimingBoot, EditTimingShutdown,  
                       RebootInstance, StopInstance,
                       DeleteInstance)
        
#        row_actions = (StartInstance, ConfirmResize, RevertResize,
#                       CreateSnapshot, SimpleAssociateIP, #AssociateIP,
#                       SimpleDisassociateIP, AttachInterface, ReallocationInstance,
#                       DetachInterface, EditInstance, AttachVolume, EditCDRom,
