...
class LaunchVolumeNG(LaunchVolume):
    ...
    verbose_name = _("Launch as Instance")
    url = "horizon:admin:volumes:index"
    classes = ("btn-launch", )
    ajax = False
...
class CreateTransfer(VolumePolicyTargetMixin, tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:create_transfer"
    classes = ("ajax-modal",)
...
class AcceptTransfer(tables.LinkAction):
    ...
    url = "horizon:admin:volumes:volumes:accept_transfer"
    classes = ("ajax-modal",)
    icon = "exchange"
...
def get_encrypted_link(volume):
    if hasattr(volume, 'encrypted') and volume.encrypted:
        return reverse("horizon:admin:volumes:volumes:encryption_detail",
