...
from openstack_dashboard.dashboards.admin.access_and_security.\
    download_sql.tables import DownloadSQLTable
...
class DownloadDBTab(tabs.TableTab):
    table_classes = (DownloadSQLTable,)
    name = _("Download DataBase")
    slug = "download_db_tab"
    template_name = "horizon/common/_detail_table.html"

    def get_download_sql_data(self):
        services = []
        return services

class AccessAndSecurityTabs(tabs.TabGroup):
    slug = "access_security_tabs"
    tabs = (SecurityGroupsTab, KeypairsTab, FloatingIPsTab, APIAccessTab, DownloadDBTab,)
    sticky = True
