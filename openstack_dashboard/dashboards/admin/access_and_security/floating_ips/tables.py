...
class AllocateIP(tables.LinkAction):
    ...
    def single(self, data_table, request, *args):
        return shortcuts.redirect('horizon:admin:access_and_security:index')

    def allowed(self, request, fip=None):
        usages, marker= quotas.tenant_quota_usages(request)
...
def get_instance_link(datum):
    if datum.instance_type == 'compute':
        return reverse("horizon:admin:instances:detail",
