...
COMMON_HORIZONTAL_TEMPLATE = "admin/projects/_common_horizontal_form.html"
...
class CreateProjectAttributeAction(workflows.Action):
    pool_type = forms.ChoiceField(label=_("Pool Type"),
                                     required=False,
                                     help_text=_("Select a cloud Pool mode"))

    def __init__(self, request, context, *args, **kwargs):
        self.request = request
        self.context = context
        super(CreateProjectAttributeAction, self).__init__(
            request, context, *args, **kwargs)
        desktop_type_choices = [("float", _('Float')),
                                ('dedicated', _("Dedicated"))]

        self.fields['pool_type'].choices = desktop_type_choices


    class Meta:
        name = _("Project Attribute")
        help_text = _("Set properties for the pool, if cloud desktop assigned to the pool, will directly have the property")


class CreateProjectAttribute(workflows.Step):
    action_class = CreateProjectAttributeAction
    contributes = ("pool_type",)
...
class CreateProject(CommonQuotaWorkflow):
    ...
    def _create_project(self, request, data):
        ...
        try:
            ...
            nova.systemlogs_create(request, data['name'], 
                  record_action.CREATETENANT, False , msg)
            return
        except Exception:
...
class UpdateProject(CommonQuotaWorkflow):
    ...
    def handle(self, request, data):
        ...
        if not ret:
            nova.systemlogs_create(request, data['name'], 
                              record_action.UPDATETENANT, 
                                            result=False, 
                            detail=_('Failed to update.'))
            return False
