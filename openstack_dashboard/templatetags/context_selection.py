...
@register.inclusion_tag('context_selection/_remote_assistance.html',
                        takes_context=True)
def show_remote_assistance(context):
    if 'request' not in context:
        return {}
    request=context['request']
    try:
        if hasattr(request.user, 'token'):
            ...
            if remote.get('remote'):
                for rem in remote['remote']:
                    rem['created_at']=datetime.strptime\
                                      (rem['created_at'], '%Y-%m-%dT%H:%M:%S.%f')
                context['remote']=remote.get('remote', {})
                context['count']=len(remote['remote'])
                return context
    except:
        return context
