...
from openstack_dashboard.api import templet_info as info


LOG = logging.getLogger(__name__)
VERSIONS = base.APIVersionManager("image", preferred_version=2)

class Templet(info.TempletInfo):
    def __init__(self, templet):
        super(Templet, self).__init__(templet)
...
def templet_download(request, image, do_checksum):
    body= Templet(glanceclient(request, '1')).download_image(image, do_checksum=do_checksum)
    filename=base.download_templet(request, body)
    info = Templet(filename).dispose_info()
    LOG.info("body ======================================%s" % filename)
    return info
