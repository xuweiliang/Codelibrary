...
def server_paging(request, search_opts=None, all_tenants=False, hyper_type="QEMU"):
    has_more = True
    has_prev = True
    #paginate = True
    ...
    if search_opts:
        search_opts = {}
    backindex = int(paginate*page_size)
    frontindex = int(backindex - page_size)
    LOG.info("paginate ===================%s" % paginate)
    if all_tenants:
    ...
    db_ins_data = servers[frontindex:backindex]
#
#    if len(servers) == backindex:
#        has_more = False
#        if paginate !=1:
#            has_prev = True
#    elif len(db_ins_data) == page_size:
#        has_more = True
#        if paginate != 1:
#            has_prev = True
#    elif len(db_ins_data) < page_size:
...
#        if paginate != 1:
#            has_prev = True
#    elif len(db_ins_data) == getattr(settings, 'API_RESULT_LIMIT',1000):
#        has_more = False
#        has_prev = False
    return (db_ins_data, has_more, has_prev, paginate)
#    return (servers, has_more, has_prev, paginate)
